...
{
    public partial class Learner_Form : Form
    {
        #region Variables
        //Camera specific
        Capture grabber;
        ...
        public Learner_Form(Form1 _Parent)
        {
            InitializeComponent();
            Parent = _Parent;
            Face = Parent.Face;
        ...
        {
            string trainedFacesPath = Path.Combine(Application.StartupPath, "TrainedFaces");
            if (Directory.Exists(trainedFacesPath))
            {
                //if (string.IsNullOrEmpty(NAME_PERSON.Text))
                //{
                    foreach (string filename in Directory.GetFiles(trainedFacesPath,"*.jpg"))
                    {
                        if (!filename.Contains("unknown"))
                            File.Delete(filename);
                    }
                    File.Copy(Path.Combine(trainedFacesPath, "TrainedLabelsBackup.xml"), Path.Combine(trainedFacesPath, "TrainedLabels.xml"), true);
                //}
                //else
                //{
                //    string[] personFiles = Directory.GetFiles(trainedFacesPath,"face_"+NAME_PERSON.Text+"*");
                //    foreach (string filename in personFiles)
                //    {
                //        if (!filename.Contains("unknown"))
                //            File.Delete(Path.Combine(trainedFacesPath, filename));
                //    }
                //}
                MessageBox.Show("Faces Deleted");
            }
        }
